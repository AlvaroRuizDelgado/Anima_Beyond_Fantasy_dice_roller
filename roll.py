...
def roll(argv):
    ...
    if (roll_type == "d100"):
        ...
        if (die_roll <= FAIL_THRESHOLD+fail_modifier):
            font_roll_color = bcolors.GREY
            font_result_color = bcolors.PURPLE
            fail_level = randint(1,100)
            roll_result = "CRITICAL FAIL!! ("+str(fail_level)+")"
        else:
            ...
            if (difficulty != 0 and die_roll < difficulty):
                ...
                if (die_roll >= ZEN_THRESHOLD):
                    roll_result = "ZEN!!"
                elif (die_roll >= INHUMAN_THRESHOLD):
                    roll_result = "INHUMAN!"
                else:
                    roll_result = "SUCCESS"
        ...
        if (die_roll < difficulty):
            ...
            roll_result = "SUCCESS (level "+str(success_level)+")"
        print(font_roll_color, die_roll, bcolors.LIGHT_RED, "-->", font_result_color, roll_result, bcolors.GREY)
        return(
                  {
                    'die_range': die_range,
...
def print_help():
    ...
    print("  - d10")
    print("Options:")
    print("  -d, --difficulty, set the difficulty.")
    print("  -m, --master, failure % is reduced by 1 (i.e. '3' is not a failure)")
    print("Examples:")
